...
from .models import League, Team, CustomUser, Match, Player
from .user_serializer import MyTokenObtainSerializer, RegisterSerializer, MatchSerializer
from .player_serializer import PlayerSerializer
from .tasks import simulate_matches_task
    ...
    def create(self, request, *args, **kwargs):
        try:
            with transaction.atomic():
                ...
                team.name = serializer.validated_data['team_name']
                team.arena = serializer.validated_data['arena_name']
                team.manager = f"{serializer.validated_data['first_name']} {serializer.validated_data['last_name']}"
                team.save()
...
@api_view(['GET'])
def real_time_match_update(request, pk):
    try:
